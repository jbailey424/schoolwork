...
def get_full_credits_for_director(dir_id): #takes director id and returns dict with the dir name, id, uri, and dict credits
                                            #dict credits has each movies title, year, uri, and notes
    def get_movie_link(mov_elm, mov_year):
        mov_links = mov_elm.find_all('a')
        for m in mov_links:
            title = m.text.strip()
            uri = m.get('href', '').strip()
            mov_year = mov_year.text.strip() if mov_year is not None else ''
            if uri.startswith('/title/tt'):
                movie_note = mov_elm.text.replace(title, '').replace(mov_year, '')
                movie_note = ' '.join(movie_note.split())
                title_id = uri.split('/')[2] #changed this function to give the movie id
                return {'title': title, 'uri': f'https://www.imdb.com{uri}', 'title_id': title_id, 'year': mov_year, 'note': movie_note}
        return {}
    ...
    time.sleep(5)  # Pause for 5 seconds after each request to avoid overloading the website
    title = ''
    try:
        soup = BeautifulSoup(html_pg, 'html.parser') #make a soup object 
...
def get_full_crew_for_movie(title_id): #takes a title id and gives a huge json-like dictionary with crew info
...
def is_feature_film(title_id): #looks for a films runtime and if its greater than 70 minutes returns True
    imdb = IMDB()
